...
function App() {
  ...
  const allDocumentsReady = REQUIRED_DOCUMENTS.every(
  ...
  const beginScreening = () => {
    ...
    if (!allDocumentsReady) {
      setIntakeError("Upload every required document before continuing.");
      setIntakeStep("documents");
      return;
    }
  ...
  const analyzeWithBackend = async () => {
    if (!intakeFiles.passport || !intakeFiles.livePhoto) {
      setBackendError("Live photo and passport are required for analysis.");
      return;
    }
    ...
    try {
      ...
      formData.append("document", intakeFiles.passport);
      ...
      <div className="app-shell scanning-page">
        ...
        <main className="scanning-main" ref={scanningRef}>
          ...
          <div className="scanning-heading">
            ...
            <p>Live person photo and required document set received. Running the requested checks.</p>
          </div>
    ...
    <div className="intake-overlay">
      <section className="intake-modal">
        ...
        <div className="intake-priority">
          ...
          <div className={`priority-step ${allDocumentsReady ? "ready" : ""}`}>
            <span>02</span>
            <div>
              <strong>REQUIRED DOCUMENTS</strong>
              <small>{allDocumentsReady ? "ALL READY" : "UPLOAD ALL 5"}</small>
            </div>
          </div>
        </div>
        ...
        <div className={`document-intake-list ${!intakeFiles.livePhoto ? "locked" : ""}`}>
          <div className="document-list-heading">
            <div>
              <span className="eyebrow">02 // REQUIRED IMAGES</span>
              <h3>Document Set</h3>
            </div>
        ...
        <div className="intake-modal-footer">
          <div>
            <strong>{intakeFiles.livePhoto && allDocumentsReady ? "READY TO SCREEN" : "INPUT INCOMPLETE"}</strong>
            <span>
              {intakeFiles.livePhoto
                ? `${REQUIRED_DOCUMENTS.filter((item) => intakeFiles[item.key]).length}/5 documents ready`
                : "Live person photo required first"}
            </span>
          </div>
          <button
            className="primary-button proceed-button"
            disabled={!intakeFiles.livePhoto || !allDocumentsReady}

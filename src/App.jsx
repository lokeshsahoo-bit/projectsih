...
const CHECK_NOTES = {
  FACE: "Ensures document owner matches the presented individual.",
};

const emptyHistory = { processed: 0, verified: 0, review: 0, highRisk: 0 };
...
const analyzeWithBackend = async () => {
  ...
  // Tally screening history whenever a new backend result arrives.
  useEffect(() => {
    if (!backendResult) return;

    const level = String(
      backendResult?.risk?.risk_level ?? "NO RESULT"
    ).toUpperCase();
    const needsReview = CHECKS.some((check) => resultCheck(check) === "REVIEW");

    setHistory((prev) => ({
      processed: prev.processed + 1,
      highRisk: prev.highRisk + (level === "HIGH" ? 1 : 0),
      review: prev.review + (needsReview && level !== "HIGH" ? 1 : 0),
      verified: prev.verified + (!needsReview && level !== "HIGH" ? 1 : 0),
    }));
  ...
  const clearHistory = () => {
    setHistory({ processed: 0, verified: 0, review: 0, highRisk: 0 });
    setBackendResult(null);
    setLastRisk(null);
        ...
        <main className="dashboard-main">
          ...
          <section className="system-overview">
            <StatCard
              ...
              value={String(history.processed)}
              note="TOTAL SCREENED"
              title="Total number of screenings run through GHOST SCAN."
            />
            <StatCard
              label="VERIFIED"
              value={String(history.verified)}
              note={history.verified ? "PASSED ALL CHECKS" : "NO SCREENINGS"}
              tone="green"
              title="Screenings that passed every check with no flags and no high-risk score."
            />
            <StatCard
              label="REVIEW REQUIRED"
              value={String(history.review)}
              note="MANUAL REVIEW"
              tone="yellow"
              title="Review Required means one or more verification checks returned REVIEW rather than PASS. The officer should manually inspect the flagged result before treating the screening as verified."
            />
            <StatCard
              label="HIGH RISK"
              value={String(history.highRisk)}
              note="FLAGGED"
              tone="red"
              title="High Risk means the AI risk engine returned a HIGH risk level. It is a strong warning signal and requires officer attention."
            />
          </section>
          ...
          <section className="dashboard-panels">
            <div className="dashboard-panel risk-panel">
              <div className="panel-title-row">
                <div>
                  <span className="eyebrow">LAST PERSON SCREENING</span>
                  <h2>Last Person Risk Score</h2>
                </div>
              ...
              <div className="risk-chart">
                ...
                <div
                  className="risk-gauge-mini"
                  style={{ "--score": `${riskPercent * 3.6}deg` }}
                >
                  <div>
            ...
            <div className="dashboard-panel quick-screening">
              ...
              <h2>Photo / Image Intake</h2>
              <p>
                Every intake begins with a mandatory live person photo,
                followed by the document image intake. Upload the images you have and continue.
              </p>
          ...
          <div className="zero-history-note">
            ...
            <strong>{history.processed} RECORDS</strong>
            <small>Processed / Verified / Review / High Risk counters above update live per screening.</small>
      ...
      <section className="intake-modal">
        <div className="intake-modal-header">
          <div>
            ...
            <h2>Photo / Image Intake</h2>
            <p>LIVE PERSON PHOTO is mandatory and must be captured first.</p>
          </div>
        ...
        <div className="intake-priority">
          ...
          <div className={`priority-step ${documentsReadyCount > 0 ? "ready" : ""}`}>
            ...
            <div>
              ...
              <small>{documentsReadyCount}/5 UPLOADED</small>
            </div>
          </div>
        </div>
        ...
        <div className={`document-intake-list ${!intakeFiles.livePhoto ? "locked" : ""}`}>
          <div className="document-list-heading">
            <div>
              <span className="eyebrow">02 // DOCUMENT IMAGES</span>
              <h3>Document Set</h3>
            </div>
        ...
        <div className="intake-modal-footer">
          <div>
            ...
            <span>
              {intakeFiles.livePhoto
                ? `${documentsReadyCount}/5 document images ready`
  ...
  return (
    <section className="risk-assessment">
      <div className="risk-assessment-header">
        ...
        <div className="risk-assessment-header-right">
          <span className={`risk-badge ${riskLevel.toLowerCase().replace(/\s+/g, "-")}`}>
            {riskLevel}
          </span>
          {onClose && (
      ...
      <div className="risk-result-grid">
        ...
        <div className="check-results">
          <div className="check-results-heading">
            <span>FETCH DETAILS // VERIFICATION STATUS</span>
...
function StatCard({ label, value, note, tone = "", title }) {
  return (
    <div className="overview-card" title={title}>
      <span>{label}</span>
      <strong>{value}</strong>

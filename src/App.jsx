...
const handleDocumentUpload = (event) => {
  ...
  if (!file.type.startsWith("image/")) {
    alert("Please upload a JPG, JPEG, PNG, or other image document.");
    event.target.value = "";
    return;
  }
      ...
      <div className="intake-modal-overlay">
        <section className="intake-modal">
          <div className="intake-modal-header">
            <div>
              <span className="panel-label">MODULE 01 // PHOTO & IMAGE INTAKE</span>
              <h2>Required Capture Sequence</h2>
              <p>
                Live person photo must be captured first. Every required
                document image must be supplied before processing can begin.
              </p>
            </div>
          ...
          <div className="intake-sequence">
            {intakeItems.map((item, index) => {
              const file = intakeFiles[item.key];

              return (
                <div
                  className={`intake-sequence-card ${
                    file ? "captured" : ""
                  } ${index === 0 ? "priority" : ""}`}
                  key={item.key}
                >
                  <div className="intake-sequence-number">
                    {String(index + 1).padStart(2, "0")}
                  </div>

                  <div className="intake-sequence-info">
                    <strong>{item.label}</strong>
                    <span>
                      {index === 0 ? "CAPTURE THIS FIRST" : item.priority}
                    </span>
                    <small>
                      {file ? file.name : "IMAGE NOT UPLOADED"}
                    </small>
                  </div>

                  <label className="intake-upload-button">
                    {file ? "REPLACE" : "SELECT IMAGE"}
                    <input
                      type="file"
          ...
          <div className="intake-modal-footer">
            <div className="intake-required-status">
              <span
                className={intakeComplete ? "ok-dot" : "warning-dot"}
              ></span>
              {intakeComplete
                ? "ALL REQUIRED IMAGES READY"
                : "ALL SIX IMAGES ARE REQUIRED"}
            </div>

            {intakeStatus && (
              <div className="intake-error">{intakeStatus}</div>
            )}
    ...
    return (
      <div className="intake-page">
        ...
        <main className="intake-main">
          ...
          <section className="input-section">
            ...
            <div className="input-grid">
              ...
              <InputCard
                ...
                description="Upload a passport, visa, ID card, or other document image from this device."
                tag="UPLOAD"
                onClick={() => {
                  selectInput({ title: "Document Upload" });
                  openDocumentUpload();
                }}
              />
          ...
          {selectedInput?.title === "Document Upload" && (
            <section className="upload-panel">
              <div className="upload-header">
                <div>
                  <span className="section-label-small">DOCUMENT INPUT // FILE 01</span>
                  <h2>Upload Document</h2>
                  <p>Select a clear image of the document for Ghost Scan analysis.</p>
                </div>
                <div className="camera-status">
                  <span className="camera-status-dot"></span>
                  {capturedImage ? "UPLOAD COMPLETE" : "UPLOAD READY"}
                </div>
              </div>

              <div className="upload-view">
                {capturedImage ? (
                  <img src={capturedImage} alt="Uploaded document" className="captured-document" />
                ) : (
                  <div className="camera-placeholder">
                    <div className="camera-placeholder-icon">FILE</div>
                    <h3>NO DOCUMENT SELECTED</h3>
                    <p>Choose a JPG, JPEG, PNG, or other image file.</p>
                    <button className="camera-button" onClick={openDocumentUpload}>SELECT DOCUMENT</button>
                  </div>
                )}
              </div>
